...
export class BaseGame {
  ...
  // Returns the player on success, { error } on a rejected reconnect, or null
  // for a plain lobby rejection (name taken / room full / unknown name).
  addPlayer(id, name, token) {
    if (this.phase !== 'lobby') {
      // Reconnect by name to resume a seat mid-game. The seat id is STABLE —
      // we never reassign it, so votes/quest cards/proposals/leader refs that
      // key off the id stay valid (no orphan ballots, no double counting).
      const existing = this.players.find((p) => p.name.toLowerCase() === name.toLowerCase());
      if (!existing) return null; // unknown name — cannot join a game in progress
      if (existing.connected) return { error: 'That seat is still connected — pick another name.' };
      // A per-seat secret token authorises reclaiming the seat (anti-hijack).
      if (existing.token && token !== existing.token) {
        return { error: 'This seat is held by another device. Rejoin from the device that took it.' };
      }
      existing.connected = true; // rebind the new socket to the existing seat id
      this.note(`${existing.name} returns to the table.`);
      return existing;
    }
