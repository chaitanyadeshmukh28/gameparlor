// Thin WebSocket client with auto-reconnect. The server is authoritative;
// we just send intents and render whatever state comes back.
...
export function useCoupSocket() {
  ...
  const queue = useRef([]);
  const rejoin = useRef(null); // {name, code} for auto-reconnect
  ...
  const connect = useCallback(() => {
    ...
    sock.onopen = () => {
      ...
      queue.current = [];
      if (rejoin.current) sock.send(JSON.stringify({ t: 'join', ...rejoin.current }));
    };
